...
function ObfuscatedEmail() {
  ...
  useEffect(() => {
    ...
    setEmail(`${policyMeta.emailUser}@${policyMeta.emailDomain}`);
  }, []);

  if (!email) {
    return (
      <span className="font-semibold text-brand-primary" aria-label="school email address">
        {policyMeta.emailUser}
        <span aria-hidden="true"> [at] </span>
        {policyMeta.emailDomain}
      </span>
    );
  }
...
// Ganti token {{email}} di dalam teks item dengan komponen email ter-obfuscate.
function renderItemText(text: string) {
  ...
  return parts.map((part, i) => (
    <Fragment key={i}>
      {part}
      {i < parts.length - 1 && <ObfuscatedEmail />}
    </Fragment>
  ));
}
...
function SectionCard({ section, ui }: { section: PolicySection; ui: PolicyUi }) {
...
export function PrivacyPolicyContent({
  ...
  const sections = policySections[locale];
    ...
    <div className="font-sans">
      ...
      <div className="grid gap-8 lg:grid-cols-[17rem_1fr] lg:items-start lg:gap-12">
        ...
        <div className="flex flex-col gap-6 md:gap-8">
          {sections.map((section) => (
            <SectionCard key={section.id} section={section} ui={ui} />
          ))}
